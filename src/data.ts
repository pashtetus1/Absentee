...
// Дальности ступеней. Одна лестница на две вещи — на марки межзвёздного
// перехода и на телескопы спутников, — и это НЕ экономия строчки, а правило
// мира: КУДА ДОЛЕТАЕШЬ, ТУДА И ВИДИШЬ. Телескоп Mk2 достаёт ровно туда же,
// куда прыжковый двигатель Mk2; чтобы разглядеть дальше, надо и лететь
// дальше, и наоборот. Пока эти два числа были разными, одно из них всегда
// оказывалось лишним: видишь дальше, чем можешь, — половина карты дразнит;
// летаешь дальше, чем видишь, — лететь некуда.
// Подобраны под кольца: 46 — до первого и вдоль него, 66 — до второго,
// 90 — до третьего, 130 — до четвёртого и пятого.
...
export const COMPS: Comp[] = [
  ...
  // Телескоп — ГЛАВНАЯ деталь карты: чужая звезда перестаёт быть точкой на
  // краю не оттого, что до неё долетели, а оттого, что на неё навели телескоп
  // со спутника. Пока телескопа не делает никто, галактика состоит из одной
  // системы — сколько бы марок перехода ни было освоено.
  //
  // ЛЕСТНИЦА ИЗ ЧЕТЫРЁХ СТУПЕНЕЙ, и дальность у ступени та же, что у марки
  // перехода того же номера (MARKRANGE): Mk1 видит на 46, как Mk1 долетает на
  // 46. Цена растёт чуть быстрее марок — смотреть дешевле, чем возить, но
  // ненамного: первая ступень рядом с буром, четвёртая дороже корпуса Mk3.
  { key:"scope1", name:"Телескоп Mk1",       short:"телескоп Mk1", diff:1500,  work:5,  base:42,  glyph:"dia", sight:MARKRANGE[0] },
  ...
  // ---- СЫРЬЁ: то, что не делают, а ДОБЫВАЮТ --------------------------------
  // Четыре вещества с астероидов, и цех не делает ни одного: их выдаёт
  // платформа, вставшая на камень своей породы (ore — сколько единиц в месяц).
  // Отсюда и вся логистика партии: камни лежат там, где лежат, а нужны они
  // там, где живут и строят, — между этими двумя местами и ходят транспортники.
  //
  //   металл  — из него цех делает ВСЕ детали: нет металла — цех стоит;
  //   вар     — на нём корабль ходит внутри системы;
  //   просинь — на ней он уходит за звёзды;
  //   энергия — её везут НА ПЛАНЕТЫ и продают людям, и это единственное сырьё,
  //             которое превращается прямо в деньги.
  //
  // ВАР И ПРОСИНЬ — это прежние «местное» и «межзвёздное топливо», и разными
  // веществами они были всегда: на варе за звезду не уйдёшь, просинь в ходовом
  // двигателе не горит. Имена им даны затем, что теперь у них есть месторождения
  // и цена своя в каждой системе, а «топливо» без уточнения перестало отвечать
  // на вопрос, какое именно.
  //
  // diff и work у сырья нули и значат ровно это: сложности нет, потому что его
  // не исследуют, и работы нет, потому что его не делают руками. В науку эти
  // ключи не попадают вовсе (allTech в tech.ts), патента на них не бывает.
  { key:"metal", name:"Металл",   short:"металл",  diff:0, work:0, base:14, glyph:"sqr", ore:1.0 },
  { key:"fuel",  name:"Вар",      short:"вар",     diff:0, work:0, base:9,  glyph:"cir", ore:1.4 },
  { key:"sfuel", name:"Просинь",  short:"просинь", diff:0, work:0, base:46, glyph:"cir", ore:0.4 },
  { key:"power", name:"Энергия",  short:"энергия", diff:0, work:0, base:8,  glyph:"dia", ore:0.7 },
...
// ---- телескопы: ступени и дальности ------------------------------------
/** Ступени телескопа, от близорукой к дальнозоркой. */
 ...
 *  смотреть нечем. Лучший, если их вдруг два. */
export function scopeRange(parts: { k: string }[]): number {
  let best = 0;
  (parts || []).forEach((p) => { best = Math.max(best, sightOfKey(p.k)); });
  return best;
}
/** Лучшая ступень телескопа, какую в галактике хоть кто-то умеет делать: её и
 *  поставят на новый спутник. null — телескопа нет ни у кого, и смотреть не на
 *  чем: партия заперта в родной системе. */
export function bestScopeMade(): string | null {
...
// ---- сырьё: то, что добывают -------------------------------------------
// Порядок в списке значим ровно там, где сырьё показывают строчками, и нигде
// больше: сперва металл (из него всё), потом два топлива, потом энергия.
export const ORES = ["metal", "fuel", "sfuel", "power"];
export function isOre(k: string): boolean { return ORES.indexOf(k) >= 0; }
/** Сколько единиц этого вещества даёт платформа в месяц; 0 — это не сырьё. */
export function oreRate(k: string): number { const c = compOf(k); return (c && c.ore) || 0; }
/** Породы астероидов — те же четыре ключа: камень ИМЕНУЕТСЯ тем, что в нём
 *  лежит, и платформа на нём добывает только это. */
export const ROCKKINDS = ORES;
/** Сколько сырья влезает в один трюм. Сырьё возят НАВАЛОМ, а не поштучно, как
 *  детали: рейс с полным трюмом металла окупает бак просини, а рейс с двумя
 *  деталями — нет (PARTS_PER_HOLD ниже). Двадцать пять — чуть больше, чем еды
 *  (FOOD_PER_HOLD 20), и это не щедрость, а пропускная способность: платформа
 *  качает год напролёт, а рейс идёт десятилетие, и с меньшим трюмом вывоз
 *  безнадёжно отстаёт от добычи — у камня копятся тысячи единиц, которые некому
 *  увезти. */
export const ORE_PER_HOLD = 25;
/** Сколько металла съедает цех на одну такую деталь. Считается от работы:
 *  трюм за 2 работы — единица металла, корпус Mk5 за 15 — пять. Сырьё металла
 *  не требует само по себе: его не делают. */
export function metalFor(f: Comp): number { return f.ore ? 0 : Math.max(1, Math.round(f.work / 3)); }
...
//
// ПРЫЖКОВОГО КОРАБЛЯ ЗДЕСЬ БОЛЬШЕ НЕТ, и корабля-открывателя тоже. Сперва
// открытие звезды было особым кораблём, потом — следствием обычного рейса: кто
// первым долетел, тот и открыл. Теперь ЗВЁЗДЫ НЕ ОТКРЫВАЮТ КОРАБЛЯМИ ВОВСЕ.
// Их открывает спутник с телескопом: он висит на орбите в своей системе и
// видит на SCOPE_RANGE вокруг. Дорога и знание разошлись — корабль везёт, а
// смотрит телескоп, — и лететь теперь можно только туда, что уже разглядели.
export const VTYPES: VType[] = [
  // Платформа больше не «жила, из которой капает доход»: она встаёт на камень
  // и выдаёт то, что в этом камне лежит (oreRate его породы), а доход — дело
  // рынка и транспорта. Поэтому выработки в рецепте нет: она у ПОРОДЫ, а не у
  // платформы, и на энергетическом камне та же платформа даёт другое.
  { key:"mine",   name:"разработка астероидов", need:{ drill:1 }, build:14, term:1800, glyph:"mine" },
  { key:"colony", name:"колония",               need:{ pod:1 },   build:22, glyph:"colony" },
  // Спутник: телескоп, и больше в рецепте ничего. КАКОЙ телескоп — решается
  // при закладке, как ступень корпуса и модель двигателя: ставят лучший, какой
  // в галактике умеют делать (shipNeed ниже), потому и в таблице его нет.
  // Боевой лазер добавляют там же и по другому поводу — спутник бывает и с
  // ним, и без него, а рецепт это то, без чего вещь не вещь.
  { key:"sat",    name:"спутник",               need:{},          build:12, glyph:"sat" },
...
/** Сколько сырья везёт корабль с такими деталями: оно идёт навалом, как еда,
 *  только плотнее — единица металла весит больше мешка зерна. */
export function oreRoomOf(parts: Part[]): number { return countOf(parts, "hold") * ORE_PER_HOLD; }
 ...
 *  груз. Платформе, колонии, спутнику и портальному добирать нечем — у них
 *  рецепт, а не грузоподъёмность, и лишнее место остаётся пустым. */
...
export const MOVES: Move[] = [
  // Способ отвечает только за ДОРОГУ. Что вокруг вообще есть — говорит телескоп
  // спутника, и говорит одинаково при обоих способах.
  { key:"drives", name:"портальные движки", comp:"drive",
    hint:"Двигатель на каждом корабле, идущем между звёздами: лететь можно к любой разглядённой звезде в пределах марки, но перевозки между звёздами дороги навсегда." },
  { key:"gates",  name:"звёздные ворота", comp:"gkit",
    hint:"Портальный корабль везёт ворота и оставляет их на маршруте к разглядённой звезде: очень дорого за каждый, зато потом по нему летают даром." }
];

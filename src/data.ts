...
export function compOf(k){ for (var i=0;i<COMPS.length;i++) if (COMPS[i].key===k) return COMPS[i]; }
...
export function colOf(k){ for (var i=0;i<COLTECH.length;i++) if (COLTECH[i].key===k) return COLTECH[i]; }
...
export function ptypeOf(k){ for (var i=0;i<PTYPES.length;i++) if (PTYPES[i].key===k) return PTYPES[i]; }
...
export function vtype(k){ for (var i=0;i<VTYPES.length;i++) if (VTYPES[i].key===k) return VTYPES[i]; }
...
export function moveOf(k){ for (var i=0;i<MOVES.length;i++) if (MOVES[i].key===k) return MOVES[i]; }
...
export function markOf(k){ for (var i=0;i<MARKS.length;i++) if (MARKS[i].key===k) return MARKS[i]; }
// пока способ не выяснен, марки называются обезличенно
export function markName(m){ return S.moveKnown ? m.name : "Межзвёздный переход " + m.short; }

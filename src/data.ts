...
export const COMPS: Comp[] = [
  ...
  // со спутника. Отсюда и цена: дешевле корпуса, дороже бура. Пока телескопа
  // не делает никто, галактика состоит из одной системы — сколько бы марок
  // перехода ни было освоено.
  { key:"scope", name:"Телескоп",            short:"телескоп",  diff:1600, work:5, base:44,  glyph:"dia" },
  ...
  { key:"laser", name:"Боевой лазер",        short:"лазер",     diff:5200, work:8, base:98,  glyph:"tri" },
...
// Докуда видит телескоп со спутника: все звёзды в этом радиусе от его системы
// становятся открыты. Число подобрано между первой и второй марками перехода
// (46 и 66): один спутник открывает больше, чем достаёт Mk1, — значит, всегда
// есть звезда, которую ВИДНО, но до которой ещё нечем долететь, и марки
// остаются лестницей. Меньше дальности Mk1 — и марки были бы не нужны вовсе:
// летать было бы некуда, кроме уже открытого.
export const SCOPE_RANGE = 60;
/** Дальность телескопа на этих деталях; 0 — телескопа нет, смотреть нечем. */
export function scopeRange(parts: { k: string }[]): number {
  return (parts || []).some((p) => { return p.k === "scope"; }) ? SCOPE_RANGE : 0;
}
...
export const VTYPES: VType[] = [
  ...
  // Спутник: телескоп, и больше в рецепте ничего. Боевой лазер к нему
  // добавляют при закладке, если в галактике его умеют делать (orders.ts) —
  // поэтому он и не стоит в таблице: спутник бывает и с ним, и без него, а
  // рецепт — это то, без чего вещь не вещь.
  { key:"sat",    name:"спутник",               need:{ scope:1 }, build:12, glyph:"sat" },

...
export function shipNeed(vt: VType, eng: string | null,
  ...
  // У СПУТНИКА ХОДОВОГО ДВИГАТЕЛЯ НЕТ. Он никуда не идёт: его собирают на
  // орбите своей планеты и там он и остаётся. Двигатель на нём был наследством
  // тех времён, когда телескоп везли к чужой звезде корабликом, — и он не
  // только занимал место в корпусе, но и запирал первый телескоп партии за
  // наукой о двигателях.
  if (vt.key !== "sat") {
    if (!eng) return null;
    n[eng] = (n[eng] || 0) + 1;

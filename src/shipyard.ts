...
import type { Corp, Part, Proposal, Shipyard, VType, World } from "./types";
...
// Без неё касса компании только копилась: к сотому году у типичной конторы
// около 500, к трёхсотому — больше тысячи, у самой богатой под три. Тратить
// это было не на что, кроме новых заказов, и очередь забивалась жилами (почти
// половина всех сборко-месяцев) просто потому, что деньги есть. А рабочие
// места филиалы просят по кошельку (labour), так что богатая контора ещё и
// уводила людей из-под верфи, и забитая очередь ползла.
//
// Теперь место в очереди стоит денег, и платят их КАЗНЕ ПЛАНЕТЫ, у которой
// стоит верфь: корабль собирают её люди, ровно как и саму верфь строит планета,
// а не казна государства. Цена — за человеко-месяцы: своей сборки и всей
// работы, что уже стоит впереди. Пустая верфь берёт почти только за работу,
// забитая — втридорога, и жила, без которой компания проживёт, в такую
// очередь не встаёт.
//
// Хлебовозы и переселенцы идут БЕЗ платы, чей бы ни был заказ: голод не должен
// упираться в цену места. Платят только за своё дело — жилу, прыжок, ворота,
// колонию. Замер показал, что для очереди это почти ничего не значит (с платой
// за всё 3.1 против 3.3 в среднем при ставке 0.25), — зато хлебовоз голодной
// колонии не ждёт, пока её правительство накопит на место.
//
// Замер, 48 партий по 300 лет, против партии без платы:
//              касса компании (медиана, 300 год)  очередей в сумме  закладка→спуск  планет  голодных колоние-месяцев
//   без платы                   1323                  15.0             375 мес.       8.7         34.7%
//   0.25                         722                   9.4             269            10.3         31.8%
//   0.5                          502                   5.7             191             9.9         26.9%
// Цена у этого двоякая. Деньги оседают у правительства родины, где строят больше
// всего (4.3 тысячи на 300 году стало 30), а тратить их ему почти не на что. И
// медленнее идёт наука: лаборатории филиалов нанимают по кошельку, и средняя
// марка освоения к 300 году по 16 сидам 2.0 против 2.5. Это принято нарочно —
// техника и так приходила быстрее, чем её успевали построить.
export const SLOT_RATE = 0.5;

/** Сколько сейчас стоит встать в очередь этой верфи со сборкой такого типа. */
export function slotPrice(y: Shipyard, vt: VType): number {
  const ahead = y.queue.reduce((a, b) => a + Math.max(0, b.left), 0);
  return Math.round(SLOT_RATE * (vt.build * YARD_WORK + ahead));
}

/** Заплатить планете за место. Кошелёк передаётся снаружи: у заказа это касса
 *  компании, у подписки на колонию — сперва остаток складчины. */
export function paySlot(y: Shipyard, price: number, pay: (sum: number) => boolean): boolean {
  if (!pay(price)) return false;
  y.world.gov.cash += price;
  return true;
}

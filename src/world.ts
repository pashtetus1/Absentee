...
export function makeWorld(body: Planet, seed: number, founder: number) {
...
export function popOf(w: World){ return w.pop.farm + w.pop.prod + w.pop.sci + w.pop.free; }
...
export function stockAt(c: Corp, sys: number, k: string){ var s = c.stock[sys]; return (s && s[k]) || 0; }
...
export function addStock(c: Corp, sys: number, k: string, n: number) {
  if (!c.stock[sys]) c.stock[sys] = {};
  c.stock[sys][k] = (c.stock[sys][k] || 0) + n;
  if (!c.tot) c.tot = {};
  c.tot[k] = (c.tot[k] || 0) + n;
}
export function totalStock(c: Corp, k: string) { return (c.tot && c.tot[k]) || 0; }
export function whereStock(c: Corp, k: string) {                 // системы, где у компании есть эта деталь
  var out = [];
  for (var s in c.stock) if (c.stock[s][k] > 0) out.push(+s);
  return out;
}
export function firstStockSys(c: Corp, k: string) {              // первая такая система, без массива
  for (var s in c.stock) if (c.stock[s][k] > 0) return +s;
  return -1;
}
export function hasBranch(c: Corp, w: World){ return w.branches.some(function (b) { return b.corp === c.id; }); }

export function openBranch(c: Corp, w: World, quiet?: boolean) {

...
export const S = {
  ...
  // Погони: сколько вольница затеяла и сколько рейсов от неё ушло. Второе
  // число и есть цена хорошего двигателя — по нему видно, что гонка настоящая.
  chases: 0, gotAway: 0,
...
// Содержание мира: сколько казна мира тратит на людей в месяц. Не рычаг —
// игрок не может это крутить, это цена существования людей.
//
// Шкала ПРОГРЕССИВНАЯ: на душу выходит UPKEEP × (1 + население / UPKEEP_SCALE),
// то есть чем мир больше, тем дороже в нём каждый человек — города, дороги,
// порядок растут быстрее числа жителей. Колония на полтора человечка платит
// почти те же 0.3 на душу (+15%), столица на двадцать — втрое больше.
//
// Зачем: казна столицы копилась без дела — 30 тысяч к 300 году. Замер, 48
// партий по 300 лет (казна столицы на 300 году / планет / голодных колоние-месяцев):
//   плоское 0.3   30.6 тыс.  10.5  25.6%
//   шкала 40      21.8       10.4  25.8%
//   шкала 20      15.6       10.3  26.9%
//   шкала 10       8.1       10.0  28.5%
// Срезать сильнее одной шкалой нельзя: население столицы весь век держится
// около двадцати, а к концу партии её казну кормит в основном плата за место
// в очереди верфи (17-21 в месяц) — постоянная прибавка к расходу за этим
// ростом не поспевает.
export const UPKEEP = 0.3;
export const UPKEEP_SCALE = 10;
/** Содержание мира в месяц при таком населении. */
export function upkeepOf(total: number): number { return total * UPKEEP * (1 + total / UPKEEP_SCALE); }

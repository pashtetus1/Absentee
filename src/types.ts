...
export interface Sat {
  ...
  /** Сколько месяцев ещё собирать. Телескоп НИКУДА НЕ ЛЕТИТ и не сходит со
   *  стапеля: его собирают прямо на орбите своей планеты, руками того филиала,
   *  что там сидит. Оттого у него и срок свой, а не место в очереди верфи. */
  left?: number;
...
export interface Warship {
  ...
  fight?: number;                 // номер боя ИЛИ погони, в которых он занят
}
...
/** ПОГОНЯ перед боем: ватага снялась с орбиты логова и идёт к рейсу.
 *
 *  Считается в единицах СИСТЕМЫ (bodyPos, galaxy.ts) — там же, где лежат миры
 *  и ходят внутрисистемные рейсы. Ватага идёт чистой погоней: каждый месяц
 *  правит на то место, где жертва сейчас, и кроет своё расстояние. Догонит —
 *  бой; жертва успеет к цели — ушла. Исход НЕ решён заранее: вольница выходит
 *  за тем, кто медленнее её, а хватит ли ей дороги, показывает сама дорога. */
export interface Chase {
  x: number; y: number;           // где ватага сейчас, в единицах системы
  px: number; py: number;         // где была в прошлом месяце — для плавного кадра
  pace: number;                   // сколько единиц кроет за месяц
}
...
export interface Fight {
  ...
  /** Пока он стоит, боя ещё НЕТ: ватага только идёт к жертве. */
  close?: Chase;
...
export interface Voyage {
  ...
  fight?: number;
  /** Номер ПОГОНИ: ватага вышла из логова и идёт к этому рейсу, но ещё не
   *  дошла. Рейс при этом ИДЁТ СВОИМ ХОДОМ — в том и гонка. Поле отдельное от
   *  fight ровно затем, чтобы движение (motion.ts) осталось слепым к погоне:
   *  стоит рейс только в бою. */
  chase?: number;
}
...
export interface Snapshot {
  ...
  /** Погонь затеяно и рейсов ушло от погони. */
  chases: number; gotAway: number;

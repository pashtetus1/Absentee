...
function revoltCrushed(g: Ground): void {
  ...
  if (c.branches.length) {
    ...
    // мир, взявшийся за оружие во второй раз, — или контора, поднявшая мятеж
    // (relief.ts). Он остаётся при своём — но разоружён, и вольницей ему
    // больше не быть. Ремесло меняется только у тех, кто родился из голода:
    // у стартовой конторы оно своё («буры», «самолёты»), и проигранный мятеж
    // не делает её артелью.
    if (c.origin) c.craft = "выживание";
    c.nerve = Math.min(c.nerve, 1.3);

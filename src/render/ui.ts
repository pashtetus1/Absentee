...
export function bindUI(): void {
  ...
  // «Заново» — ТА ЖЕ партия с начала: тот же сид, та же галактика, тот же
  // способ перелёта. Новую партию с новым сидом даёт вход на страницу по адресу
  // без сида (start).
...
function start(): void {
  ...
  // Отложенная партия продолжается только по адресу С ЕЁ СИДОМ — так страница
  // открывается после F5 и по закладке, сделанной во время игры: сид в адрес
  // пишет live(). Ссылка с другим сидом сильнее отложенной: по ней пришли за
  // конкретной партией. А вход БЕЗ сида — это новая партия с новым сидом:
  // «Заново» больше не меняет сид, и другого способа начать другую галактику
  // нет. Отложенная в обоих случаях теряется — сохранение в игре одно.
  //   Несовместимое сохранение выше этой развилки: его полоса показывается при
  // любом адресе, потому что затереть чужую партию молча нельзя никогда.
  if (!on || want === undefined || want !== seedOf()) build(undefined, want);
  live();
  if (!framing) { framing = true; requestAnimationFrame(frame); }
}

/** Партия с начала по кнопке (или по вписанному сиду): прежней больше нет,
 *  поэтому и отложенная стирается — иначе следующий запуск развернул бы её
 *  поверх новой. Без сида берётся новый. */

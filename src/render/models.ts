...
export function ship(kind: string, x: number, y: number, s: number, rot: number, col: string): void {
  ...
  if (kind === "jump") {
    ...
    // Военный корабль: узкий клин с двумя стволами вперёд и скошенной кормой.
    // Форма отвечает на «что это» (журнал), и отвечать она обязана с одного
    // взгляда: среди грузовиков-коробок и круглых модулей на карте должно быть
    // видно, что вот это — вооружённое, ещё до того, как по нему ткнули.
    poly([0,-9, 1.9,-3.4, 1.9,4.2, -1.9,4.2, -1.9,-3.4]);          // корпус
    poly([-5.4,-1.2, -3.2,-1.2, -3.2,5.4, -5.4,5.4]);              // борт
    poly([3.2,-1.2, 5.4,-1.2, 5.4,5.4, 3.2,5.4]);
    poly([-4.9,-6.8, -3.7,-6.8, -3.7,-1.4, -4.9,-1.4]);            // стволы
    poly([3.7,-6.8, 4.9,-6.8, 4.9,-1.4, 3.7,-1.4]);
    cx.fillStyle = "#0b1120"; poly([-1.1,-2.6, 1.1,-2.6, 1.1,-0.6, -1.1,-0.6]);
  } else {

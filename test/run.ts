...
test("камни не ложатся под планеты, а места створов свободны", () => {
  ...
  // своём кольце 420.
  //   ОДНА ПЛОХАЯ ГАЛАКТИКА ИЗ ОКНА ДОПУСКАЕТСЯ, и это не поблажка. Расстановка
  // перебирает углы 300 раз и, если места так и не нашлось, сдаётся и ставит
  // створ на луч — примерно одна галактика из шестидесяти. Прежде тест стоял
  // на окне сидов, и КАЖДАЯ правка правил (две детали в COMPS сдвигают поток
  // случайных чисел ещё до расстановки) перекладывала окно заново. Теперь
  // считается число накладок: одна — цена того самого «сдаюсь», две и больше —
  // сломанная расстановка, и это тест поймает.
  ...
  assert(soft.length <= 1, "накладок в двенадцати галактиках " + soft.length + ": " + soft.join("; "));
});
...
test("опустевший мир выпадает из государства, и за ним ничего не тянется", () => {
  ...
  [13, 44, 47, 3, 12, 16, 17].forEach((seed) => {
    const sim = load("dist/index.html", { seed });

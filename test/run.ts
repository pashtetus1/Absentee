...
import type { Corp, FlyAcct, Ground, Lot, Part, Pop, Snapshot, Voyage, World } from "../src/types.ts";
...
test("створы ведут ко всем соседям в конусе, а маршруты складываются из створов", () => {
  ...
  // Сиды перебраны вместе со всеми, когда мятеж стал битвой, а вольница вышла на
  // рейсы у себя дома: скан 40 сидов по 300 лет даёт общие створы в семи
  // партиях, эти три — из самых наглядных (8, 3 и 2 общих створа).
  for (const seed of [32, 31, 11]) {
    const sim = load("dist/index.html", { seed });
...
// ТЕЛЕСКОП НИКУДА НЕ ЛЕТИТ. Его собирают на орбите СВОЕЙ планеты руками того
// филиала, что на ней сидит: ни верфи, ни очереди, ни рейса, ни ходового
// двигателя. Раньше он был кораблём — сходил со стапеля, а в чужую систему его
// вёз паром, — и оттого первый телескоп партии был заперт за наукой о
// двигателях и о топливе, без которой в партии нет вообще ничего, кроме Тиры.
//
// Проверяется всё разом: спутник стоит только там, где у конторы есть люди; в
// его деталях нет ходового; ни один рейс и ни один кораблик не везёт спутник.
test("телескоп собирают на своей орбите, а не привозят", () => {
  let built = 0, seenBuilding = 0;
  for (const seed of [2, 4, 3]) {
    const sim = load("dist/index.html", { seed });
    const seen = new Set<object>();
    runYears(sim, 200, (st) => {
      st.voyages.forEach((v) => {
        assert(v.cargo !== "sat", "спутник кто-то везёт рейсом");
        assert(!v.sat, "у рейса на борту спутник");
      });
      st.systems.forEach((s) => {
        s.ships.forEach((sh: { kind: string }) => {
          assert(sh.kind !== "sat", "спутник идёт корабликом внутри системы");
        });
        s.sats.forEach((x) => {
          if (!x.live) { seenBuilding++; assert((x.left || 0) > 0, "строящийся спутник без срока"); }
          assert(!x.parts.some((p) => /^eng\d$/.test(p.k)), "на спутнике ходовой двигатель");
          // В момент ЗАКЛАДКИ у конторы обязаны быть свои люди в системе:
          // телескоп собирают они, привезти его неоткуда. Потом филиал может и
          // уйти — сооружение на орбите от этого никуда не денется, и требовать
          // филиал от УЖЕ СТОЯЩЕГО спутника значило бы проверять не то правило.
          if (seen.has(x)) return;
          seen.add(x);
          const mine = s.bodies.some((b) => b.world && b.world.branches.some((br) => br.corp === x.owner));
          assert(mine, st.corps[x.owner].name + ": спутник заложен у " + s.name + ", где у неё нет филиала");
        });
      });
    });
    built += sim.state().systems.reduce((a, s) => a + s.sats.filter((x) => x.live).length, 0);
  }
  assert(built > 0, "за три партии не встал ни один телескоп");
  assert(seenBuilding > 0, "ни одного спутника не застали на сборке");
});

// Спутник заказывают и свозят к нему детали, как к платформе. Рецепт у него
// один — телескоп; боевой лазер прибавляют при закладке, и он тоже обязан
// доезжать до готовой вещи.
test("спутник собирают из телескопа, а оружие на нём бывает", () => {
  ...
  // — 16 партий под воротами. Сиды — первые два из тех, где он есть; пересажены,
  // когда мятеж стал битвой, а вольница вышла на рейсы у себя дома.
...
// ТЕЛЕСКОП У СПУТНИКА СВОЙ. Не лучший в галактике, а лучший, какой умеет сама
// контора: спутник — её глаза, и видит он ровно настолько, насколько она дошла
// своей наукой. Купить чужую дальнозоркость нельзя — можно купить карту.
test("спутник несёт телескоп своей конторы, а не лучший в галактике", () => {
  let checked = 0;
  for (const mode of ["drives", "gates"]) {
    const sim = load("dist/index.html", { seed: 23 });
    sim.build(mode, 23);
    const st = runYears(sim, 250);
    st.systems.forEach((s) => s.sats.forEach((x) => {
      const scope = x.parts.filter((p) => /^scope\d$/.test(p.k))[0];
      assert(scope, "спутник без телескопа");
      assert(st.corps[x.owner].known[scope.k],
             mode + ": " + st.corps[x.owner].name + " поставила спутник с чужим телескопом " + scope.k);
      checked++;
    }));
  }
  assert(checked > 0, "за две партии не встал ни один спутник");
});

// ВООРУЖЁННЫЙ СПУТНИК ДЕРЁТСЯ — как сооружение, а не как корабль: он не
// гонится и не отступает, он стоит там, где его поставили. За жертву встают
...
// Сид подобран: вооружённый спутник встречается примерно в половине партий, а
// спутник ВОЛЬНИЦЫ, прикрывший свой же промысел, — реже. На четвёртом за
// триста лет набирается шесть боёв со спутниками, из них один за вольницу.
test("спутник вступает в бой, который идёт у него под носом", () => {
  ...
  runYears(sim, 300, (st) => {
    st.fights.forEach((f) => {
      if (seen.has(f.id) || f.close) return;          // погоня — ещё не бой, стороны не собраны
      seen.add(f.id);
      const inAtt = f.att.filter((x) => x.sat), inDef = f.def.filter((x) => x.sat);
      if (!inAtt.length && !inDef.length) return;
      met++;
      inAtt.concat(inDef).forEach((x) => {
        assert(!x.ship, "спутник в бою числится ещё и кораблём");
        assert(x.dmg > 0, "спутник в бою без урона: безоружных в драку не зовут");
        assert(x.sat.armed, "в бой попал спутник без оружия");
      });
      // Спутник, попавший в бой, обязан стоять В ЭТОЙ ЖЕ системе. Круга
      // дальности больше нет и быть не может: промысел идёт только внутри
      // звезды (battle.ts), и «под самым носом» значит буквально это.
      inAtt.concat(inDef).forEach((x) => {
        assert(x.sat.sys === f.sys,
               "в бой влез спутник из " + st.systems[x.sat.sys].name + ", а дерутся у " + st.systems[f.sys].name);
      });
    });
    // Сбитого спутника на орбите быть не должно.
    st.systems.forEach((s) => s.sats.forEach((x) => {
      assert(x.hp > 0 || !x.live, s.name + ": сбитый спутник всё ещё висит на орбите");
    }));
  });
  }
  // За ВОЛЬНИЦУ спутник тоже встаёт (satsFor зовёт своих на сторону нападающих),
  // но привязать это к сиду нельзя: ватага сама заводит телескоп редко, и на
  // двенадцати просмотренных партиях по триста лет не завела ни разу. Правило
  // остаётся в коде и в этой записи; проверять его сидом значило бы держать
  // проверку, зелёную по случайности.
  assert(met > 0, "за две партии по триста лет ни один спутник не вступил в бой");
});

// Спутник КРУЖИТ ВОКРУГ ПЛАНЕТЫ, а не у звезды: прежде у каждого была своя
// орбита вокруг светила, и телескопы в столичной системе висели прямо на
// солнце. Проверяется, что планета у спутника из ЕГО системы и что орбита
// лежит в разумных дорожках: шесть шагов от планеты — столько спутников у
// одной планеты не набирается ни в одной партии.
test("спутник кружит вокруг планеты, а не на солнце", () => {
  let checked = 0;
  for (const seed of [6, 8]) {
    const st = runYears(load("dist/index.html", { seed }), 200);
    st.systems.forEach((s) => s.sats.forEach((x) => {
      assert(x.body, s.name + ": спутник " + st.corps[x.owner].name + " ни вокруг чего не кружит");
      assert(s.bodies.indexOf(x.body) >= 0, s.name + ": спутник кружит вокруг планеты из чужой системы");
      assert(x.orb >= x.body.rad + 44 - 0.01 && x.orb <= x.body.rad + 44 + 11 * 6,
             s.name + ": орбита спутника " + x.orb.toFixed(1) + " не похожа на дорожку у планеты");
      checked++;
    }));
  }
  assert(checked > 0, "за две партии не встал ни один спутник");
...
// Две орбиты одного радиуса вокруг одной планеты — это два телескопа, которые
// никогда не разойдутся на экране: они так и будут ехать один в другом.
test("спутники у одной планеты идут разными дорожками", () => {
  for (const seed of [6, 8, 11]) {
    const st = runYears(load("dist/index.html", { seed }), 200);
    st.systems.forEach((s) => s.sats.forEach((a, i) => s.sats.forEach((b, j) => {
      if (j <= i || a.body !== b.body) return;
      assert(Math.abs(a.orb - b.orb) > 0.01,
             s.name + ": два спутника на одной орбите вокруг " + (a.body ? a.body.name : "звезды"));
    })));
  }
});
...
test("колонии в основном выживают", () => {
  ...
  // ЖИВЫМ, и это цена бунта, а не вымирание. Поднят до половины тогда, когда
  // спутники стали носить СВОЙ телескоп: галактика за партию стала теснее
  // (4-5 систем вместо 7), колоний в ней 2-5 штук, и одна мелкая колония
  // качает дробь на десять процентов. Широкий замер, 20 сидов по 200 лет:
  // 27 мелких из 70, то есть 38.6% — ровно столько же, сколько было до
  // спутников; на восьми сидах разброс доходит до половины.
  //   Настоящее вымирание мерится не этим, а следующей строкой.
  assert(dead <= cols * 0.5, "вымерло " + dead + " из " + cols + " колоний");
  ...
  // значит прятать разницу. По широкому скану (20 сидов, 70 колоний) пустых
  // было НОЛЬ. С охотой вольницы у себя дома (battle.ts, HOME_HUNT) хлебовоз до
  // колонии в системе логова иногда не доходит: 24 сида по 200 лет — 5 пустых
  // из 111, 4.5%. Порог оставлен с запасом и ловит именно вымирание.
...
// случайности (см. журнал). Здешние — после того, как мятеж стал битвой, а
// вольница вышла на рейсы у себя дома: скан сорока сидов — 15 подходящих, взяты
// первые три.
...
// ГИПЕР НЕПРИКОСНОВЕНЕН. Между звёздами корабль идёт в створе или в прыжке —
// там его нет, и вольница до него не дотягивается. Раньше она брала его
// именно там: логово стоит В СИСТЕМЕ, перегон выходит из неё же, кольцо охоты
// накрывало его целиком, а рейс идёт двести с лишним месяцев — из системы не
// выпускали почти никого. Со стороны это и выглядело так, будто корабли просто
// исчезают по дороге между звёздами.
//
// Проверка смотрит не на слова в ленте, а на сами бои: у каждого жертва обязана
// быть рейсом ВНУТРИ системы.
test("вольница не трогает межзвёздный перегон", () => {
  let checked = 0;
  for (const seed of [3, 6, 1]) {
    const sim = load("dist/index.html", { seed });
    runYears(sim, 300, (st) => {
      st.fights.forEach((f) => {
        if (!f.prey) return;
        checked++;
        assert(f.prey.sysFrom === undefined, "вольница взяла рейс на гипере (сид " + seed + ")");
        assert(f.prey.from && f.prey.to && f.prey.from.sys === f.prey.to.sys,
               "вольница взяла рейс между звёздами (сид " + seed + ")");
      });
    });
  }
  assert(checked > 0, "за три партии по триста лет вольница не взялась ни за один рейс");
});

// ПОГОНЯ — ГОНКА, а не задержка перед неминуемым боем. Ватага выходит за тем,
// кто медленнее её, но успеет ли она к нему до того, как он сядет, заранее не
// знает никто. Проверяется, что случается и то и другое: и догнали, и ушли.
test("рейс уходит от погони, если успевает к своей планете", () => {
  let chases = 0, away = 0, caught = 0;
  for (const seed of [3, 6, 1, 11]) {
    const sim = load("dist/index.html", { seed });
    const st = runYears(sim, 300);
    chases += st.chases; away += st.gotAway; caught += st.battles;
  }
  assert(chases > 0, "за четыре партии ни одной погони");
  assert(away > 0, "от погони не ушёл ни один рейс: гонка решена заранее");
  assert(caught > 0, "погони есть, а до боя ни одна не дошла");
  assert(away + caught <= chases, "ушло и догнали вместе больше, чем было погонь");
});

// Перехват называет МЕСТО ПРОИСШЕСТВИЯ, а не логово вольницы. Сводка писала
// «сбила рейс командира X у Тира» по системе логова — то есть про рейс,
// взятый у чужой звезды, говорила, что он пропал у своей. Проверка идёт по
// самим рейсам, а не по словам в ленте: в строке обязана стоять та звезда, у
// которой всё случилось.
test("перехват называет место, а не логово вольницы", () => {
  let checked = 0;
  for (const seed of [3, 6, 1]) {
    const sim = load("dist/index.html", { seed });
    ...
    runYears(sim, 300, (st) => {
      ...
      watch = new Map();
      st.voyages.forEach((v) => {
        if (v.fight === undefined || v.sysFrom !== undefined) return;
        watch.set(v, "у " + st.systems[v.from.sys].name);
      });
    });
  }
  assert(checked > 0, "за три партии по триста лет ни один рейс не сбили");
});
...
// Мятеж конторы — ВОССТАНИЕ С НАЗЕМНОЙ БИТВОЙ, а не смена вывески. Раньше
// стартовая контора становилась вольницей в тот же день, и чужие филиалы
// оставались работать в её логове бок о бок с ватагой. Стерегутся обе половины:
// стартовая контора уходит в разбой только через битву, и в логове, взятом
// битвой, нет ни одной стартовой конторы, кроме самой вольницы, — ни сразу, ни
// потом (branchTrade туда больше не пускает). Артель самих жителей там быть
// может: это люди логова, а не выгнанные.
test("мятеж конторы идёт наземной битвой, и из взятого логова чужих выгоняют", () => {
  let battles = 0, taken = 0;
  for (const seed of [1, 3]) {
    const sim = load("dist/index.html", { seed });
    const fought = new Set<number>(), live = new Map<Ground, number>(), lairs = new Map<number, World>();
    runYears(sim, 250, (st) => {
      live.forEach((corp, g) => {
        if (st.grounds.indexOf(g) >= 0) return;
        live.delete(g);
        const c = st.corps[corp];
        if (c.pirate && c.home === g.world) { lairs.set(corp, g.world); taken++; }
      });
      st.grounds.forEach((g) => {
        if (live.has(g) || st.corps[g.corp].origin !== undefined) return;
        live.set(g, g.corp); fought.add(g.corp); battles++;
      });
      st.corps.forEach((c) => {
        if (c.pirate && c.origin === undefined)
          assert(fought.has(c.id), "сид " + seed + ": " + c.name + " ушла в разбой без битвы");
      });
      lairs.forEach((w, corp) => {
        const c = st.corps[corp];
        if (!c.pirate || c.home !== w) { lairs.delete(corp); return; }
        w.branches.forEach((b) => {
          assert(b.corp === corp || st.corps[b.corp].origin !== undefined,
                 "сид " + seed + ": в логове " + w.body.name + " работает " + st.corps[b.corp].name);
        });
      });
    });
  }
  assert(battles > 0, "ни одного мятежа битвой");
  assert(taken > 0, "ни одно логово не взято мятежом");
});
...
// Заодно проверяются два главных правила промысла, и они противоположны.
// Рейс, за который ДЕРУТСЯ, стоит: иначе он спокойно долетит посреди боя, и
// весь спор окажется ни о чём. Рейс, за которым ГОНЯТСЯ, идёт: в этом вся
// гонка, и остановить его значило бы сделать погоню пустой формальностью
// перед неминуемым боем.
test("вольница строит корабли и дерётся за добычу, а рейс в бою стоит", () => {
  let battles = 0, raids = 0, raiders = 0, held = 0, frozen = 0, ran = 0, chases = 0;
  for (const seed of [3, 6, 11]) {
    const sim = load("dist/index.html", { seed });
    ...
    runYears(sim, 300, (st) => {
      ...
      st.fights.forEach((f) => {
        if (f.close) {
          assert(f.att.length > 0, "погоня без ватаги");
          assert(f.prey && f.prey.chase === f.id, "рейс под погоней не помечен её номером");
          assert(f.prey.fight === undefined, "рейс под погоней помечен ещё и боем");
          return;
        }
      ...
      st.voyages.forEach((v) => {
        if (v.fight === undefined) {
          // Под погоней рейс обязан ИДТИ: гонка на том и стоит.
          if (v.chase !== undefined && was.has(v) && v.t < 1) {
            assert(v.t > was.get(v), "рейс стоял, пока за ним шла погоня");
            ran++;
          }
          was.set(v, v.t); return;
        }
    ...
    const st = sim.state();
    battles += st.battles; raids += st.raids; chases += st.chases;
  }
  assert(raiders > 0, "за три партии вольница не построила ни одного корабля");
  assert(chases > 0, "корабли есть, а на перехват никто не выходит");
  assert(battles > 0, "погони есть, а до боя ни одна не дошла");
  ...
  assert(ran > 0, "рейсы под погоней ни разу не проверены на ходу");
});

// РЕЙДЕР ДОВООРУЖАЕТСЯ. Ватага строит три корабля и на том успокаивается
// (RAID_CAP), а первые её годы — это как раз те годы, когда оружия нет нигде:
// лучемёт осваивают позже, чем вспыхивает первое восстание. Оттого самоделка
// оставалась с уроном голого корпуса весь век, притом что в самом логове через
// полвека лежал лучемёт. Проверяется и то, что оружие на рейдере ВЫТЕСНЯЕТ
// броню: место в корпусе Mk1 одно, а броня в галактике дешевле и раньше, и
// первой на это место всегда попадала она.
test("вольница довооружает рейдер тем, что появилось в логове", () => {
  let armed = 0, bare = 0, grew = 0;
  for (const seed of [1000, 1077, 1154]) {
    const sim = load("dist/index.html", { seed });
    const room: Record<string, number> = {};
    sim.consts.COMPS.forEach((c) => { if (c.slots) room[c.key] = c.slots; });
    const roomOfParts = (parts: Part[]): number => {
      const h = parts.filter((pt) => room[pt.k])[0];
      return h ? room[h.k] : parts.length;
    };
    const was = new Map<number, number>();
    runYears(sim, 200, (st) => {
      st.warships.forEach((w) => {
        if (w.owner < 0 || !st.corps[w.owner].pirate) return;
        const had = was.get(w.id);
        if (had !== undefined && w.dmg > had + 1e-9) grew++;
        was.set(w.id, w.dmg);
      });
    });
    const st = sim.state();
    st.warships.forEach((w) => {
      if (w.owner < 0 || !st.corps[w.owner].pirate) return;
      const beam = w.parts.filter((pt) => /^beam\d$/.test(pt.k));
      const armour = w.parts.filter((pt) => /^armor\d$/.test(pt.k));
      if (beam.length) armed++; else bare++;
      assert(beam.length || !armour.length || w.parts.length < roomOfParts(w.parts),
             "в тесном корпусе броня заняла место лучемёта");
      assert(w.parts.length <= roomOfParts(w.parts), "в рейдер влезло больше деталей, чем мест в корпусе");
    });
  }
  assert(grew > 0, "за три партии ни один рейдер не стал бить сильнее, чем в день сборки");
  assert(armed > bare, "рейдеров без оружия больше, чем вооружённых: довооружение не работает");
});

// Вольница бьёт и рейсы МЕЖДУ МИРАМИ СВОЕЙ системы. Раньше такие пропускались
// вовсе, и ватага, чьё логово в одной системе со столицей, годами стояла на
// орбите, не выходя ни разу, — а в начале партии вся жизнь внутри одной звезды.
// И только у себя: рейс между мирами чужой системы не её дело.
test("вольница выходит и на рейсы внутри своей системы, но только своей", () => {
  let inside = 0;
  for (const seed of [1, 2]) {
    ...
    runYears(sim, 250, (st) => {
      st.fights.forEach((f) => {
        ...
        seen.add(f.id);
        const v = f.prey;
        if (!v || v.sysFrom !== undefined || v.from.sys !== v.to.sys) return;
        inside++;
        const p = st.corps[f.raider];
        assert(p && p.home && p.home.sys === v.from.sys,
               "вольница вышла на рейс между мирами чужой системы " + st.systems[v.from.sys].name);
      });
    });
  }
  assert(inside > 0, "за две партии вольница ни разу не вышла на рейс внутри своей системы");
});
...
test("военный бюджет доходит до арсеналов и полиции", () => {
  ...
  // Сиды пересажены, когда мятеж стал битвой, а вольница вышла на рейсы у себя
  // дома: казённый корабль генерал строит не в каждой партии (скан двадцати
  // сидов — в десяти), а арсеналы почти в каждой. Эта пара — первые из тех, где
  // есть и то, и другое.
  for (const seed of [2, 4]) {
    const sim = load("dist/index.html", { seed });
    sim.setLever("army", 360);
...
test("видов рейсов ровно столько, сколько знает панель", () => {
  ...
  // деталями, переселенческий) встречаются не на всякой партии. Скан троек:
  // на [3,8,59] видов три, на этой — пять.
...
// Сид 3 доводит партию до второго государства к 131 году — раньше всех
// просмотренных; прежний сид 7 до отделения не доходит с тех пор, как телескоп
// перестал летать и голод пошёл иначе.
test("сцена рисуется и после того, как государств стало несколько", () => {
  const sim = load("dist/index.html", { withDom: true, seed: 3 });
  let drawn = 0;
...
test("панель рейса разбирает порожний перегон и грузовик с деталями", () => {
  ...
  // Скан сидов: рейс, по которому есть чем щёлкнуть, встречается далеко не на
  // всяком — галактика стала теснее, и деталей между звёздами возят меньше.
  const sim = load("dist/index.html", { withDom: true, seed: 9 });
  ...
  for (let i = 0; i < 300 * 12 && !(empty && parts); i++) {
    ...
    if (!parts) {
      ...
      if (v) {
        ...
        // Карта сама держит известное в кадре, поэтому точка карты переводится
        // в точку экрана через камеру — так же, как её переводит сцена.
        const cam = sim.camera();
        sim.__click("view", { clientX: cam.x + ((1-u)*(1-u)*A.x + 2*(1-u)*u*cpx + u*u*B.x) * cam.k,
                              clientY: cam.y + ((1-u)*(1-u)*A.y + 2*(1-u)*u*cpy + u*u*B.y) * cam.k });
...
test("сохранение переживает отделение, разбой и опустевшие миры", () => {
  // Сид 3 доводит партию до второго государства (см. проверку гербов): там
  // появляются поля, которых в начале партии нет вовсе — герб, своя казна,
  // логово вольницы, военные корабли, погони и бои. Ровно их и теряет
  // небрежное сохранение.
  const a = load("dist/index.html", { seed: 3 });
  runYears(a, 300);
  const st = a.state();

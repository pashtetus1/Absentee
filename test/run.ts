...
// Спутник висит У ПЛАНЕТЫ, а не у звезды: прежде у каждого была своя орбита
// вокруг светила, и телескопы в столичной системе висели прямо на солнце.
// Допуск — шесть шагов от планеты: столько спутников у одной планеты не
// набирается ни в одной партии.
test("спутник висит у планеты, а не на солнце", () => {
  let checked = 0;
  for (const seed of [6, 8]) {
    const st = runYears(load("dist/index.html", { seed }), 200);
    st.systems.forEach((s) => s.sats.forEach((x) => {
      const sx = Math.cos(x.ang) * x.r, sy = Math.sin(x.ang) * x.r;
      assert(s.bodies.some((b) => Math.hypot(Math.cos(b.ang) * b.r - sx, Math.sin(b.ang) * b.r - sy) <= b.rad + 44 + 11 * 6),
             s.name + ": спутник " + st.corps[x.owner].name + " висит далеко от всех планет");
      checked++;
    }));
  }
  assert(checked > 0, "за две партии не встал ни один спутник");

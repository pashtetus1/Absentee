...
test("спутник вступает в бой, который идёт у него под носом", () => {
  ...
  runYears(sim, 300, (st) => {
    st.fights.forEach((f) => {
      ...
      // Спутник, попавший в бой, обязан стоять близко: «под самым носом» — это
      // тот же круг, в котором промышляет вольница.
      inAtt.concat(inDef).forEach((x) => {
        const s = st.systems[x.sat.sys];
        assert(Math.hypot(s.x - f.x, s.y - f.y) <= 45,
               "в бой влез спутник из " + s.name + ", а он далеко");
      });
    });
  ...
  assert(met > 0, "за триста лет ни один спутник не вступил в бой");
  assert(sides > 0, "вольница ни разу не прикрыла промысел своим спутником");
});

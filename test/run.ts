...
test("переселение случается", () => {
  let moved = 0;
  const each: string[] = [];
  // Сиды пересажены после появления войны: партия целиком определяется сидом,
  // и любая правка правил двигает расход случайности (см. журнал).
  for (const seed of [5, 8, 11]) {
...
test("панель рейса разбирает порожний перегон и грузовик с деталями", () => {
  // Сид пересажен после войны: грузовик с деталями между звёздами теперь ещё и
  // сбивают, и на прежнем сиде за триста лет не нашлось ни одного, по которому
  // можно было бы честно щёлкнуть.

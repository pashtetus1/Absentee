...
// выполнен всегда. Здесь стоит сторож на этот случай: при CUT = 0.35 родина
// доходила до 12 100 к сотому году, и тест бы упал.
test("казна мира не разгоняется до бессмысленных величин", () => {
  for (const seed of [2, 67, 71]) {
    let peak = 0;
...
test("к открытию перелётов родина заполнена на проценты, а не наполовину", () => {
  ...
  // Порог 0.25, а не 0.15: с тягой цены еды к резерву родина перестала голодать
  // и растёт чуть быстрее — 9-21% по восьми сидам против прежних 6-16%. Это
  // по-прежнему «на проценты», и рост держит g = 0.0012, а не голод.
  assert(fillAtOpen < 0.25, "родина заполнена на " + Math.round(fillAtOpen * 100) + "% к открытию перелётов");
});
...
// в две промышленные руки берётся ещё реже: пересчёт по двум дюжинам сидов
// дал 4 (290г), 5 (250г), 6 (236г), 22 (292г), 24 (229г) — пять из двадцати
// четырёх против прежней половины. Дыра известна и записана в журнал (п. 7):
// единственное решение игрока о верфи за партию висит на этом пороге.
// Отсюда сиды и горизонт в триста лет. Стенд отвечает за игрока политикой
...
test("отказ возвращает взносы и поднимает долю компаний", () => {
  const sim = load("dist/index.html", { seed: 6 });
  sim.setApproval("manual");
...
test("одобренная верфь строится, а при отказах остаётся одна домашняя", () => {
  const yes = load("dist/index.html", { seed: 6 }); yes.setApproval("always");
  runYears(yes, 300);
  assert(yes.state().shipyards.length > 1, "при одобрении всего за триста лет второй верфи так и нет");
  const no = load("dist/index.html", { seed: 6 }); no.setApproval("never");
...
test("предложение ждёт ответа, а компании перебивают его выгодным", () => {
  ...
  // остаётся мало лет: перебор шестидесяти сидов дал ровно один такой (47,
  // год 275). Отсюда и сиды: 6 и 24 — самые ранние предложения, 47 — торг.
  let outbid = 0;
  for (const seed of [6, 24, 47]) {
    const sim = load("dist/index.html", { seed });
    sim.setApproval("manual");
